# -*- coding: utf-8 -*-
"""Base de données programmatique du plan comptable SYSCOHADA révisé."""
...
SYSCOHADA_RAW = """# PLAN COMPTABLE GÉNÉRAL SYSCOHADA RÉVISÉ - LISTE COMPLÈTE
...
- 292 PROVISIONS POUR DÉPRÉCIATION DES TERRAINS
    - 2921 Provisions pour dépréciation des terrains agrícolas et forestiers
...
- 293 PROVISIONS POUR DÉPRÉCIATION DES BÂTIMENTS, INSTALLATIONS TECHNIQUES ET AGENCEMENTS
    - 2931 Provisions pour dépréciation des bâtiments industriels, agricoles, administratifs et commerciaux sur sol propre
    - 2932 Provisions pour dépréciation des bâtiments industriels, agricoles, administratifs et commerciaux sur sol d'autrui
...
- 477 PRODUITS CONSTATÉ'S D'AVANCE
...
- 625 PRIMES D'ASSURANCE
    ...
    - 6253 Assurances pour risques d'exploitation
...
- 658 CHARGES DIVERSES
    ...
    - 6582 Dons
    - 6583 Mécénat
- 659 CHARGES PROVISIONנées D'EXPLOITATION
...
- 753 QUOTE-PART DE RÉSULTAT SUR EXÉCUTION PARTIELLE DE CONTRATS PLURIEXERCICES
...
**Cette liste comprend maintenant TOUS les comptes du Plan Comptable Général SYSCOHADA révisé, avec leur structure hiérarchique complète.**
"""


@dataclass(frozen=True)
class SyscohadaAccount:
    """Représente un compte unique du plan SYSCOHADA."""
    ...
    def to_dict(self) -> Dict[str, str]:
        return {
            ...
            "classe": self.classe,
            "classe_label": self.classe_label,
...
CLASS_RE = re.compile(r"^##\s+\*\*CLASSE\s+(\d)\s*:\s*(.+?)\*\*", re.IGNORECASE)
RUBRIQUE_RE = re.compile(r"^###\s+\*\*(.+?)\*\*", re.IGNORECASE)
SECTION_RE = re.compile(r"^####\s+\*\*(.+?)\*\*", re.IGNORECASE)
BOLD_LINE_RE = re.compile(r"^\*\*(.+?)\*\*$")
ACCOUNT_RE = re.compile(r"^-+\s*(\d{2,4})\s+(.+)$")
...
def _clean_label(text: str) -> str:
    clean = text.strip()
    clean = re.sub(r"\s+\([^)]*\)$", "", clean) if clean.endswith(")") else clean
    return clean.strip()


def parse_syscohada_plan(raw_text: str = SYSCOHADA_RAW) -> List[SyscohadaAccount]:
    entries: List[SyscohadaAccount] = []
    current_classe: Optional[int] = None
    current_classe_label: Optional[str] = None
    current_rubrique: Optional[str] = None
    ...
    for raw_line in raw_text.splitlines():
        ...
        classe_match = CLASS_RE.match(line)
        if classe_match:
            current_classe = int(classe_match.group(1))
            current_classe_label = _clean_heading(classe_match.group(2))
            current_rubrique = None
            current_section = None
            continue

        rubrique_match = RUBRIQUE_RE.match(line)
        if rubrique_match and not line.startswith("####"):
            current_rubrique = _clean_heading(rubrique_match.group(1))
            current_section = None
            continue
        ...
        bold_line_match = BOLD_LINE_RE.match(line)
        if bold_line_match and not ACCOUNT_RE.match(line):
            current_section = _clean_heading(bold_line_match.group(1))
            continue

        account_match = ACCOUNT_RE.match(line)
        if account_match and current_classe is not None and current_classe_label:
            numero = account_match.group(1)
            libelle = _clean_label(account_match.group(2))
            path = [p for p in (current_classe_label, current_rubrique, current_section) if p]
            entries.append(
                SyscohadaAccount(
                    numero=numero,
                    intitule=libelle,
                    classe=current_classe,
                    classe_label=current_classe_label,
                    rubrique=current_rubrique,
...
@lru_cache(maxsize=1)
def get_syscohada_accounts() -> List[SyscohadaAccount]:
    return parse_syscohada_plan()


SYSCOHADA_ACCOUNTS: List[SyscohadaAccount] = get_syscohada_accounts()
SYSCOHADA_INDEX: Dict[str, SyscohadaAccount] = {account.numero: account for account in SYSCOHADA_ACCOUNTS}


def get_account(numero: str) -> SyscohadaAccount:
    try:
        return SYSCOHADA_INDEX[numero]
    except KeyError as exc:  # pragma: no cover - defensive
        raise KeyError(f"Compte SYSCOHADA introuvable: {numero}") from exc


def search_accounts(prefix: str) -> List[SyscohadaAccount]:
    prefix = prefix.strip()
    return [account for account in SYSCOHADA_ACCOUNTS if account.numero.startswith(prefix)]


def export_syscohada_json(destination: Path) -> Path:
    destination.parent.mkdir(parents=True, exist_ok=True)
    data = [account.to_dict() for account in SYSCOHADA_ACCOUNTS]
...
__all__ = [
    ...
    "SYSCOHADA_ACCOUNTS",
    "SYSCOHADA_INDEX",
    "get_syscohada_accounts",
    "get_account",
    "search_accounts",
    "export_syscohada_json",
]


if __name__ == "__main__":  # pragma: no cover
    print(f"Comptes SYSCOHADA chargés: {len(SYSCOHADA_ACCOUNTS)}")
